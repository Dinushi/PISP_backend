...
import com.wso2.finance.open.banking.pisp.factories.BankConnectionApiServiceFactory;
...
public class BankConnectionApi {
    ...
    @io.swagger.annotations.ApiOperation(value = "Add connection to a bank API",
            notes = "The PISP will register as a TPP in ASPSP and admins will add required details " +
                    "to the PISP system to connect with the bank exposed APIs", response = void.class)
    ...
    public Response addANewBank(@ApiParam(value = "Chosen content type", required = true,
            allowableValues = "{values=[application/json]}") @HeaderParam("Content-Type") String contentType,
                                @ApiParam(value = "The session id is set in the cookie", required = true,
                                        allowableValues = "{values=[application/json]}")
                                @HeaderParam("Cookie") String cookie,
                                @ApiParam(value = "Information relevent to the bank", required = true)
                                            BankInfoDTO body) {
    ...
    @Produces({"application/json"})
    @io.swagger.annotations.ApiOperation(value = "get list of banks supported by PISP", notes = "",
            response = DebtorBankDTO.class, responseContainer = "List")
    @io.swagger.annotations.ApiResponses(value = {
    ...
    @Produces({"application/json"})
    @io.swagger.annotations.ApiOperation(value = "Remove the bank from pisp service",
            notes = "This to pisp admins to remove a bank from its supported banks list.", response = void.class)
    @io.swagger.annotations.ApiResponses(value = {
    ...
    public Response removeBank(@ApiParam(value = "The id of the debtor bank as used in pisp", required = true,
            allowableValues = "{values=[application/json]}") @PathParam("bank-uid") String bankUid,
                               @ApiParam(value = "The session id is set in the cookie", required = true,
                                       allowableValues = "{values=[application/json]}")
                               @HeaderParam("Cookie") String cookie) {
