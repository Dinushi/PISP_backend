...
public class AccessTokenManagementDAO {
     ...
     */
    public InternalResponse getLastApplicationToken(String bankID) {
        ...
        try (Connection connection = JDBCPersistenceManager.getInstance().getDBConnection()) {
            try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
                ...
                try (ResultSet rs = preparedStatement.executeQuery()) {
                    ...
                    if (rs.next()) {
                        return new InternalResponse(new AccessToken(rs.getString("token"),
                                formatter.parse(rs.getString("valid_till"))), true);
                    } else {
                        return new InternalResponse(ErrorMessages.NO_RECORD_FOUND, false);
                    }
